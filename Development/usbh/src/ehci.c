...
static void ehciHandler(void)
{
  ...
  uint32_t portStat = EHCI->PORTSC1;
  ...
  if (usbSts & USBHS_USBSTS_PCI_MASK){
    ...
    EHCI->PORTSC1 = portStat;
    if (portStat & USBHS_PORTSC1_CSC_MASK){
      if (st_EhciInterrupt.ehciPortConnectStatChgCb){
        st_EhciInterrupt.ehciPortConnectStatChgCb();
      }
    }
    if (portStat & USBHS_PORTSC1_PEC_MASK){
      if (st_EhciInterrupt.ehciPortEnDisCb){
        st_EhciInterrupt.ehciPortEnDisCb();
      }
    }
    if (portStat & USBHS_PORTSC1_OCC_MASK){
      if (st_EhciInterrupt.ehciPortOvCurrCb){
        st_EhciInterrupt.ehciPortOvCurrCb();
      }
    }
  } 
